...
$queryTransDetail = mysqli_query($koneksi, "SELECT trans_order.order_code, trans_order.order_date, trans_order.order_end_date, trans_order.order_status, jenis_service.service_name, jenis_service.price, trans_order_detail.* FROM trans_order_detail LEFT JOIN jenis_service ON jenis_service.id = trans_order_detail.id_service LEFT JOIN trans_order ON trans_order.id = trans_order_detail.id_order WHERE trans_order_detail.id_order = '$id'");

$rowTransDetail = [];
while ($rowDetail = mysqli_fetch_assoc($queryTransDetail)) {
    $rowTransDetail[] =  $rowDetail;
}
// $rowTransDetail = mysqli_fetch_all($queryTransDetail, MYSQLI_ASSOC);
// print_r($rowTransDetail);
// die;
...
$queryService = mysqli_query($koneksi, "SELECT id, service_name, price FROM jenis_service");
...
if (isset($_POST['simpan'])) {
    ...
    $order_end_date = $_POST['order_end_date'];  // Tambahkan field untuk order_end_date jika ada
    $order_status = $_POST['order_status'];  // Jika ada order status
    $total_price = $_POST['total_price'];
    $order_pay = $_POST['order_pay'];  // Jika ada status pembayaran
    $order_change = $_POST['order_change'];  // Jika ada perubahan order

    // Ambil data service dan qty
    $id_service = $_POST['id_service'];
    $qty = $_POST['qty'];
    $notes = $_POST['notes'];  // Ambil notes jika ada

    // Insert data ke tabel trans_order
    $insertTransOrder = mysqli_query($koneksi, "INSERT INTO trans_order (id_customer, order_code, order_date, order_end_date, order_status, total_price, order_pay, order_change) 
    VALUES ('$id_customer', '$order_code', '$order_date', '$order_end_date', '$order_status', '$total_price', '$order_pay', '$order_change')");
    ...
    foreach ($id_service as $key => $value) {
        ...
        $queryPaket = mysqli_query($koneksi, "SELECT id, price FROM jenis_service WHERE id = '$id_service'");
    ...
    $pickup_date = $_POST['pickup_date'];  // Ambil tanggal pickup jika ada
    $pickup_notes = $_POST['pickup_notes'];  // Ambil notes pickup jika ada

    // Menyimpan data ke trans_laundry_pickup (jika ada input untuk pickup)
    if (!empty($pickup_date)) {
        $insertPickup = mysqli_query($koneksi, "INSERT INTO trans_laundry_pickup (id_order, id_customer, pickup_date, notes, created_at, updated_at) 
        VALUES ('$last_id', '$id_customer', '$pickup_date', '$pickup_notes', NOW(), NOW())");
    }
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-sm-6" align="right">
                                                    ...
                                                    <?php if ($rowTransDetail[0]['order_status'] == 0): ?>
                                                        <a href="tambah-transaksi-pengambilan.php?ambil=<?php echo $rowTransDetail[0]['id_order'] ?>" class="btn btn-warning">Ambil Cucian</a>
                                                    <?php endif ?>
                                            ...
                                            <div class="card-body">
                                                <table class="table table-bordered table-striped">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th><?php echo isset($rowTransDetail[0]['order_end_date']) ? date('d-m-Y', strtotime($rowTransDetail[0]['order_end_date'])) : 'N/A'; ?></th>
                                                    </tr>
                                                    <tr>
                                                        <th>Status</th>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3 row">
                                            ...
                                            <div class="col-sm-6">
                                                <label for="order_end_date" class="form-label">Tanggal Berakhir Order</label>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select id="id_service" class="form-control">
                                                        ...
                                                        <?php while ($dataService = mysqli_fetch_assoc($queryService)): ?>
                                                            <option value="<?php echo $dataService['id'] ?>" data-price="<?php echo $dataService['price'] ?>"><?php echo $dataService['service_name'] ?></option>
                                                        <?php endwhile ?>
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <!-- Button Tambah Paket -->
                                        <div class="mb-2">
                                            <button class="btn btn-primary btn-sm mb-2 add-row">Tambah Paket</button>
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                        <th>No</th>
                                                ...
                                                <tfoot>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="number" class="total-harga form-control" name="total_price" id="total_price" readonly>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="number" class="dibayar form-control" name="order_pay" id="dibayar">
                                                        </td>
                                                    </tr>
                                                    <!-- Kembalian -->
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="number" class="kembalian form-control" name="order_change" id="kembalian" readonly>
                                                        </td>
                                                    </tr>
